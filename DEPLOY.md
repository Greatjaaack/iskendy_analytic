...
Копится с 26.08: **24 коммита аналитики** (`dd745c2` → `HEAD master`; свежие с кодом — `3409676` адаптер
Saby, `1c8a1e4` журнал и проверка сессии iiko, `07cd90c` фронт: «сегодня» по Москве и
чистка, `d092276` мёртвый код бэкенда и общий `daterange`, затем фиксы локального
прогона: каналы = KPI, «Без категории», «Сегодня» без 500) и **10 коммитов табло**
...
- [ ] Коммит на сервере = последний коммит `master` на GitHub
- [ ] «Чеки и выручка по типу обслуживания» за месяц = KPI (чеки и рубли), «Состав чека»: товарных чеков = KPI
- [ ] Страница P&L и «График и ФОТ» открываются, цифры те же, что до деплоя (затронуты циклы по дням)
