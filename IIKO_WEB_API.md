...
⚠️ **`OrderNum` уникален только внутри дня** — нумерация начинается заново каждое утро
(за 01–20.09.2026 всего 237 разных номеров на 3 310 заказов). Любая группировка «по
заказам» за период длиннее суток обязана включать `OpenDate.Typed`, иначе заказы разных
дней склеятся в один. В коде ключ заказа собирают `order_group_fields`/`split_order_row`
(`services/olap_parse.py`).
...
группировка `[OpenDate.Typed, OrderNum, DishCategory, DishName]`, у каждого заказа берём
«Статус» и относим к нему все его блюда (см. `/api/dishes/service-breakdown`). field0
склеивает группы через «, ». На одном заказе «Статусов» может быть **несколько** (кассир
отметил и «Доставка», и «С собой» — заказ 151 от 20.09.2026): канал выбирается по
приоритету доставка > с собой > в зале (`utils.stronger_channel`), а не по порядку строк.
