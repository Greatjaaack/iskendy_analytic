...
### Фронт
React 19 + Vite + TS, **react-router** (роуты в `App.tsx`, layout с `Sidebar`). Страницы в `src/pages/`: `Dashboard`, `Suppliers`, `SupplierCard`, `NewSupplier`, `Nomenclature`, `TtkCard`. Компоненты дашборда (что показывает каждый — в `README.md`): `KpiCards` (дельты к пр. периоду), `RevenueChart` (режимы Сумма/По статусам/Погода, тип графика, фильтр дней, погода+дельта на оси X), `HourlyChart` (Сумма/По статусам), `ChecksDistribution`, `ServiceBreakdown` (блюдо×канал, сортировка), `CheckComposition` (состав чека), `CheckFullness` (наполненность чеков), `HourlyBreakdown`, `DishTable` (группировка/сортировка/поиск/с-с% — сам тянет данные), `Sidebar`. Выбор периода — пресет или диапазон (`RangeSel` в `api.ts`, хелперы `rangeQS`/`rangeKey`). Запросы — `src/api.ts`, типы — `src/types.ts`, форматтеры — `src/format.ts`, валидация (телефон/email) — `src/validation.ts`; **пороги/интервалы/палитра — `src/constants.ts`**.
