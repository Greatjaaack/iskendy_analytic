...
Мапим только ₽-суммы постоянных статей и ставки-% в `PnlMonth`. ФОТ НЕ импортируем —
он считается из графика смен (`schedule.labor_for_period`); списания/упаковка живут
на дневном уровне (`PnlDayCost`), из таблицы берём лишь помесячную упаковку как резерв.
Админ-ФОТ входит в «Прочие» P&L, поэтому склеивается с «Прочие ₽».
"""
