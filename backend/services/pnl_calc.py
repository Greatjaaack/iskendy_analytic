"""Расчёт P&L за период: выручка и food cost из БД, постоянные затраты — помесячно.

Структура повторяет финансовую модель ресторана (Google Sheet): выручка и food cost
считаются автоматически, постоянные затраты (аренда, коммуналка, админ-ФОТ, маркетинг)
вводятся на месяц и аллоцируются на день делением на календарные дни — поэтому отчёт
честен для любого периода. Переменные статьи (списания, упаковка, химия, расходники)
вводятся по дням. Ставки-% (налог УСН, комиссия агрегатора, мотивация) применяются к
выручке.

Лесенка прибыли: EBITDA = выручка − себестоимость производства (COGS + операционный ФОТ)
− операционный OPEX, то есть ДО налога и кап-резерва; ниже EBITDA вычитаются налог УСН и
кап-резерв, и получается чистая прибыль.

Выручка берётся тем же путём, что и на дашборде (`services/revenue_source`), иначе отчёты
разошлись бы между собой; прошлый период для сравнения сдвигается на целую неделю, чтобы
сопоставлять одинаковые дни недели.

Вынесено из `routers/pnl.py` (этап 7а аудита): 500 строк расчёта в роутере нельзя было
тестировать иначе как через HTTP.
"""

import calendar
from datetime import date  # noqa: F401  (используется в схемах роутера)
...
def default_month(year: int, month: int) -> dict:
...
def row_to_dict(row: PnlMonth) -> dict:
    ...
    with SessionLocal() as db:
        for r in db.execute(select(PnlMonth)).scalars():
            if (r.year, r.month) in needed:
                out[(r.year, r.month)] = row_to_dict(r)
    return out


def load_day_costs(df: date, dt: date) -> dict[str, dict]:
...
def day_var_costs(d: date, day_costs: dict, months: dict) -> dict:
    ...
    m = months.get((d.year, d.month)) or default_month(d.year, d.month)
...
def _day_pnl(
    ...
    **Маркетинг исключён из подневного разреза** (решение пользователя — это лумповый
    ...
    m = months.get((d.year, d.month)) or default_month(d.year, d.month)
    dim = calendar.monthrange(d.year, d.month)[1]
    var = day_var_costs(d, day_costs, months)
...
async def build_pnl(
    period: str, df: date, dt: date, is_custom: bool, include_delivery: bool
) -> dict:
    """Собрать P&L за период (без HTTP-слоя)."""
    ...
    rate_m = months.get((dt.year, dt.month)) or default_month(dt.year, dt.month)
    ...
    for dd in days:
        ...
        mm = months.get((dm.year, dm.month)) or default_month(dm.year, dm.month)
    ...
    if prev_dates_sorted:
        ...
        prev_months = load_months(prange_from, prange_to)
        prev_labor_day = labor_by_day(prange_from, prange_to)
        prev_day_costs = load_day_costs(prange_from, prange_to)
