...
# Подпись позиций без меню-категории. У точки это напитки комбо «Айран_», «Кола_»,
# «Кола б/с_» (с 28.03.2026 — около четверти всех проданных штук).
...
# Приоритет канала, когда на заказе несколько «Статусов» (кассир отметил и «Доставка»,
# и «С собой» — так есть, например, у заказа 151 от 20.09.2026). Раньше побеждал тот,
# что встретился в ответе последним, то есть результат зависел от порядка строк.
# Правило: доставка сильнее всего (от неё зависят деньги — комиссия агрегатора),
# затем «с собой», в зале — слабейший (он же значение по умолчанию).
CHANNEL_PRIORITY = {CHANNEL_DELIVERY: 3, CHANNEL_TAKEAWAY: 2, CHANNEL_DINEIN: 1}
